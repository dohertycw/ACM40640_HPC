...
int main(void){
	...
	#pragma omp parallelnum_threads(5) shared(A,B,n,m,p,q) private(i,j)
	{
	printf("A: \n");
	#pragma omp task
	{
	...
	#pragma omp parallel for schedule(dynamic) shared(nSamples,n,q,C) private(i,j,h)
...
double dot(double **A, double **B, int n, int q, int p){
	int i, j, k;
	double prod = 0.0;
	#pragma omp parallel for shared(A, B, n, q, p) private(i,j,k) reduction(+:prod)
	for(k = 0; k < p; k++){
		prod += A[n][k]*B[k][q];
	}
	return prod;
