...
int main(void){
  ...
  scanf("%d",&n);
  if (n<2 || n>1000) {
    printf(" Enter a positive number in range 2<n<1001\n");
    exit(1);
  }
...
#pragma omp  parallel num_threads(5) private(i,j,tid), shared(nthreads,n,a)
{
  ...
  /*Here we ensure that each thread executes in sequence using omp for ordered, meaning that the threads pass through here sequentially*/
